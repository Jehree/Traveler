...
export class TravelController{
    ...
    updateOffraidPos(exitName:string, statuses:any, statusesPath:string, profile:IAkiProfile):string{
        ...
        if (isDeath){
                ...
                for (let i = profileItems.length; i > 0; i--){
                    ...
                    (
                        ...
                        if (config.checkpoint_letter_consume_upon_death && whereToRespawn !== config.home){
                            ...
                            if (profileInventory.items[i-1].upd?.Key?.NumberOfUsages !== undefined){

                                profileInventory.items[i-1].upd.Key.NumberOfUsages += 1
                            } else {
                            ...
                            if (profileInventory.items[i-1].upd?.Key?.NumberOfUsages >= usageMax){
            ...
            } else {whereToRespawn = config.home}
        ...
        return statuses.offraid_position
    }
