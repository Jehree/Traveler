...
export class MiscUtils{
    ...
    uninstallTraveler(profile:IAkiProfile, profileFolderPath:string, statusesPath:string):void{
        ...
        Stash_Controller.saveToStashFile(profileInventory.items, statuses.offraid_position, profileFolderPath)
    ...
    backupProfile(logger:ILogger, profileId:string, trvProfileFolderName:string, backupType:string, addInfo:string):void{
        ...
        try {
            ...
            logger.log(`[Traveler]: ${profileId}.json file and ${trvProfileFolderName} folder backed up to the "Traveler/profiles/.profile backups" folder!`, "green")
    ...
    markItemsOnPmcFIR(exitName:string, profile:IAkiProfile):void{
        ...
        if (!isDeath){
            for (const item in profileItems){
                const itemUpd = profileItems[item].upd
                const itemParentId = profileItems[item].parentId
                const blacklistedItems = Get_Data.firTweakItemBlacklist()

                if (!blacklistedItems.includes(profileItems[item]._tpl)){
                    if (itemParentId !== undefined && itemParentId !== "hideout"){
                        if (itemUpd !== undefined){
                            itemUpd.SpawnedInSession = true 
                        } else {
                            profileItems[item]["upd"] = {SpawnedInSession: true}
                        }
                    }
                }
            }
